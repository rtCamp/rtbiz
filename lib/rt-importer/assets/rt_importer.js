jQuery(document).ready(function () {
	...
	jQuery.rtImporter = {
		...
		initPostData: function () {
			jQuery('#map_mapping_import').on('click', function () {
			...
			jQuery(document).on('change', '.map_form_fixed_fields', function (e) {
				...
				if (arr_map_fields[field_name].type !== undefined && arr_map_fields[field_name].type !== 'defined') {
					...
					if (arr_map_fields[field_name].type === 'key') {
						...
						jQuery(this).parent().append(DOMPurify.sanitize(tmpStr));
					}
					$defaultInput = jQuery('<input>', {
						type: 'text',
						name: 'default-' + field_name,
						value: ''
					});

					jQuery(this).parent().next().html($defaultInput);
					...
					tmpStr += template(tmpArr) + '</select>';
					jQuery(this).parent().next().html(DOMPurify.sanitize(tmpStr));
					if (jQuery(this).next().length < 1) {
						$mapLink = jQuery('<a>', {
							'data-field-name': field_name,
							href: '#mapField',
							'data-map-data': arr_map_fields[field_name].definedsource,
							'data-field': this.name.replace('field-', '')
						}).text(' Map ');
						jQuery(this).after($mapLink);
