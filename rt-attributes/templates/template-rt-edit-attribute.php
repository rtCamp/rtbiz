				...
				<tr class="form-field form-required">
					...
					<td>
						<select name="attribute_store_as" id="attribute_store_as">
							<option
								value="taxonomy" <?php selected( $att_store_as, 'tax' ); ?>><?php _e( 'Taxonomy' ); ?></option>
							<option
								value="meta" <?php selected( $att_store_as, 'meta' ); ?>><?php _e( 'Meta Value' ); ?></option>
					...
					<td>
						<select name="attribute_render_type" id="attribute_render_type">
							<optgroup label="Taxonomy">
								<option
									value="autocomplete" <?php selected( $att_render_type, 'autocomplete' ); ?>><?php _e( 'Autocomplete' ); ?></option>
								<option
									value="dropdown" <?php selected( $att_render_type, 'dropdown' ); ?>><?php _e( 'Dropdown' ); ?></option>
								<option
									value="checklist" <?php selected( $att_render_type, 'checklist' ); ?>><?php _e( 'Checklist' ); ?></option>
								<option
									value="radio" <?php selected( $att_render_type, 'radio' ); ?>><?php _e( 'Radio' ); ?></option>
								<option
									value="rating-stars" <?php selected( $att_render_type, 'rating-stars' ); ?>><?php _e( 'Rating Stars' ); ?></option>
							</optgroup>
							<optgroup label="Meta">
								<option
									value="date" <?php selected( $att_render_type, 'date' ); ?>><?php _e( 'Date' ); ?></option>
								<option
									value="datetime" <?php selected( $att_render_type, 'datetime' ); ?>><?php _e( 'Date & Time' ); ?></option>
								<option
									value="currency" <?php selected( $att_render_type, 'currency' ); ?>><?php _e( 'Currency' ); ?></option>
								<option
									value="text" <?php selected( $att_render_type, 'text' ); ?>><?php _e( 'Text' ); ?></option>
								<option
									value="richtext" <?php selected( $att_render_type, 'richtext' ); ?>><?php _e( 'Rich Text' ); ?></option>
							</optgroup>
					...
					<td>
						...
						<?php foreach ( $all_post_types as $pt ) { ?>
							<label><input type="checkbox"
							              name="attribute_post_types[]" <?php echo esc_html( ( in_array( $pt->name, $att_post_types ) ) ? 'checked="checked"' : '' ); ?>
				...
				<tr>
					...
					<td>
						<?php foreach ( $att_post_types as $pt ) { ?>
							...
							<a target="_blank"
							   href="<?php echo esc_html( admin_url( 'edit-tags.php?taxonomy=' . $this->get_taxonomy_name( $att_name ) . '&post_type=' . $pt ) ); ?>"
							   class="button configure-terms"><?php echo $labels->name . ' ';
								_e( 'Terms' ); ?></a>
						<?php } ?>
