...
include_once RT_BIZ_PATH . 'lib/rt-lib.php';
include_once RT_BIZ_PATH . 'admin/helper/rt-biz-functions.php';
...
if ( ! class_exists( 'Rt_Biz' ) ) {
	...
	class Rt_Biz {
		...
		function includes() {
			require_once plugin_dir_path( __FILE__ ) . '/vendor/' . 'redux/ReduxCore/framework.php';
			...
			$rtb_app_autoload = new RT_WP_Autoload( RT_BIZ_PATH . 'admin/' );
		...
		function update_database() {
			$updateDB = new RT_DB_Update( trailingslashit( RT_BIZ_PATH ) . 'index.php', trailingslashit( RT_BIZ_PATH . 'admmin/schema/' ) );
			$updateDB->do_upgrade();
		}
		...
		function load_styles_scripts() {
			global $rt_contact, $rt_company;
			wp_enqueue_script( 'rt-biz-admin', RT_BIZ_URL . 'admin/assets/javascripts/admin.js', array( 'jquery' ), RT_BIZ_VERSION, true );
			...
			if ( isset( $_REQUEST['post'] ) && isset( $_REQUEST['action'] ) && 'edit' == $_REQUEST['action'] ) {
				...
				if ( in_array( $post_type, array( $rt_contact->post_type, $rt_company->post_type ) ) ) {
					...
					wp_enqueue_script( 'rt-biz-admin-validation', RT_BIZ_URL . 'admin/assets/javascripts/validation.js', array( 'jquery' ), RT_BIZ_VERSION, true );
				}
			}
		...
		function register_menu() {
			...
			$logo_url               = ! empty( $settings['logo_url']['url'] ) ? $settings['logo_url']['url'] : RT_BIZ_URL . 'admin/assets/img/biz-16X16.png' ;
		...
		function plugins_dependency_enque_js() {
			wp_enqueue_script( 'rtbiz-plugins-dependency', RT_BIZ_URL . 'admin/assets/javascripts/rtbiz_plugin_check.js', '', false, true );
