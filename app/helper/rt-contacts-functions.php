...
function rt_contacts_get_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {

	if ( $args && is_array($args) )
		extract( $args );

	$located = rt_contacts_locate_template( $template_name, $template_path, $default_path );

	do_action( 'rt_contacts_before_template_part', $template_name, $template_path, $located, $args );

	include( $located );

	do_action( 'rt_contacts_after_template_part', $template_name, $template_path, $located, $args );
}

function rt_contacts_locate_template( $template_name, $template_path = '', $default_path = '' ) {

	global $rt_contacts;
	if ( ! $template_path ) $template_path = $rt_contacts->templateURL;
	if ( ! $default_path ) $default_path = RT_CONTACTS_PATH_TEMPLATES;
	...
	return apply_filters('rt_contacts_locate_template', $template, $template_name, $template_path);
}
