...
if ( ! class_exists( 'Rt_Biz_Dashboard' ) ) {
	...
	class Rt_Biz_Dashboard {
		...
		public function __construct() {
			$this->rt_biz_setup_defaults();
			Rt_Biz::$loader->add_action( 'wp_ajax_update_rtbiz_welcome_panel', $this, 'rt_biz_update_welcome_panel_ajax' );

			/* Setup Google Charts */
			Rt_Biz::$loader->add_action( 'rtbiz_after_dashboard', $this, 'rt_biz_render_google_charts' );

			/* Metaboxes for dashboard widgets */
			Rt_Biz::$loader->add_action( 'rt_biz_dashboard_add_meta_boxes', $this, 'rt_biz_add_dashboard_widgets' );

			Rt_Biz::$loader->add_action( 'rtbiz_welcome_panel', $this, 'rt_biz_welcome_panel' );

			Rt_Biz::$loader->add_action( 'rtbiz_after_dashboard', $this, 'rt_biz_print_dashboard_js' );
		}
		...
		public function rt_biz_update_welcome_panel_ajax() {
		...
		public function rt_biz_setup_dashboard() {
			/* Add callbacks for this screen only */
			Rt_Biz::$loader->add_action( 'load-' . $this->screen_id, $this, 'rt_biz_page_actions', 9 );
			Rt_Biz::$loader->add_action( 'admin_footer-' . $this->screen_id, $this, 'rt_biz_footer_scripts' );

			Rt_Biz::$loader->run();

			$this->rt_biz_check_welcome_panel();
		}
		 ...
		 */
		public function rt_biz_page_actions() {

			if ( isset( $_REQUEST['page'] ) && self::$page_slug === $_REQUEST['page'] ) {
				do_action( 'rt_biz_dashboard_add_meta_boxes', $this->screen_id, null );
		...
		public function rt_biz_render_google_charts() {
			...
			foreach ( $relations as $r ) {
				...
				if ( 'taxonomy' == $attr->attribute_store_as ) {
					add_meta_box( 'rtbiz-people-by-' . $attr->attribute_name,  $contact_labels['name'] . __( ' by ' ) . $attr->attribute_label, array(
						$this,
						'rt_biz_dashboard_widget_content',
		...
		public function rt_biz_dashboard_site_activity() {
			...
			$future_posts = $this->rt_biz_dashboard_recent_posts( array(
				'max'    => 5,
				'status' => 'publish',
				'order'  => 'ASC',
		...
		public function rt_biz_dashboard_recent_posts( $args, $post_type ) {
			$query_args = array(
				'post_type'      => $post_type,
		...
		public function rt_biz_dashboard_recent_comments_row( &$comment, $show_date = true ) {
		...
		public function rt_biz_dashboard_recent_comments( $total_items = 5 ) {
			...
			if ( $comments ) {
				...
				foreach ( $comments as $comment ) {
					$this->rt_biz_dashboard_recent_comments_row( $comment );
				}
				echo '</div> </div>';
			} else {
				return false;
		...
		public function rt_biz_team_by_contacts( $obj, $args ) {
			...
			$rows[] = array( __( 'Uncategorized' ), $this->rt_biz_get_post_count_excluding_tax( $taxonomy, $post_type ) );
		...
		//todo: check data empty
		public function rt_biz_offering_wise_contacts( $obj, $args ) {
		...
		//todo: check data empty
		public function rt_biz_dashboard_widget_content( $obj, $args ) {
			...
			$rows[] = array( __( 'Others' ), $this->rt_biz_get_post_count_excluding_tax( $taxonomy, $post_type ) );
		...
		public function rt_biz_get_post_count_excluding_tax( $taxonomy, $post_type ) {
