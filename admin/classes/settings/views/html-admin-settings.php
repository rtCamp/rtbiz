...
<div class="wrap rtbiz">
	<form method="<?php echo esc_attr( apply_filters( 'rtbiz_settings_form_method_tab_' . $current_tab, 'post' ) ); ?>"
	      id="mainform" action="" enctype="multipart/form-data">
		<div class="icon32 icon32-rtbiz-settings" id="icon-rtbiz"><br/></div>
		<h2 class="nav-tab-wrapper rtbiz-nav-tab-wrapper">
			<?php
		...
		<?php
		if ( ! empty( $setting_page_url ) ) {
			$href = $setting_page_url;
		} else {
			$href = admin_url( 'edit.php?post_type=' . $slug . '&page=rtbiz-' . $slug . '-settings' );
		}
		do_action( 'rtbiz_sections_' . $current_tab, $href );
		...
		<p class="submit">
			<?php if ( ! isset( $GLOBALS['hide_save_button'] ) ) : ?>
				<input name="save" class="button-primary" type="submit"
				       value="<?php esc_attr_e( 'Save changes', 'rtbiz' ); ?>"/>
			<?php endif; ?>
